...
//		applog(LOG_INFO, "Failed to get n2size in ");
  ...
  free(pool->nonce1);
	...
	applog(LOG_NOTICE, "Pool %d coin change requested", pool->pool_no);
...
bool subscribe_extranonce(struct pool *pool)
{
	...
	if (!res_val || json_is_false(res_val) || (err_val && !json_is_null(err_val)))  {
		...
		if (err_val) {
			...
			if (pool->extranonce_subscribe && strcmp(ss, "Method 'subscribe' not found for service 'mining.extranonce'") == 0) {
